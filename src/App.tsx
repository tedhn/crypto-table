...
const sourceCode = `import { IconChevronDown } from "@tabler/icons-react";
import {
  Button,
  Dropdown,
  Input,
...
function App() {
  ...
  // check if filter query exists, if so filter data | changes every time filter query / data changes
  useEffect(() => {
    if (filterQuery) {
      const filtered = data.filter((coin) =>
        coin.name.toLowerCase().includes(filterQuery.toLowerCase())
  ...
  return (
    <section className="p-4 sm:px-6 md:px-8 lg:px-10 xl:px-12">
      ...
      <section>
        <h1 className="mb-8 text-2xl font-bold">App source code</h1>
